...
Future<ToolResponse> sendToLLM(List<FunctionDescription> functions,
  ...
  Map<String, dynamic>
      globalResponse; // so the response is still visible in the catch block below
  try {
    // test3 key
...
// then throw an exception.
      throw Exception('LLM http call failed: ${response.statusCode}');
    }
  } catch (e) {
...
Future<void> main() async {
  ...
  var futureFunctionCall = await sendToLLM([
    FunctionDescription(
        name: 'beantwoord_algemene_vraag',
